...
const validSettings = ["iife", "module", "inline", "single"];

const WORKER_SCHEMA = "sharedWorker";

export function sharedWorker(): Plugin {
  return {
    ...
    async load(id) {
      ...
      if (settings.includes("iife") && settings.includes("module"))
        throw new Error(
          'You can not have both "module" and "iife" settings in a Worker File!'
        );

      const realID = rest.join(":");

      const buildType = isBuild
        ? settings.includes("module")
          ? "module"
          : "iife"
        : settings.includes("iife")
        ? "iife"
        : "module";
